...
IMPORTANT naming note: the course Prometheus is *shared* across groups, and these
``delivery_*`` names are unprefixed by group. To avoid one group's series colliding with
another's, Grafana queries pin the ``instance`` label to our host:port. The metric
*definitions* here are group-agnostic; the disambiguation happens at query time.
"""
...
# service stateless — the JSONL files written by the CD hook are the source of truth.
...
# Retrain outcome for the last deploy that queued one. It's filled in asynchronously by
# watch_dag once Airflow finishes, so it can legitimately read "running" for a while.
...
def _iso_to_epoch(value: str) -> float:
    ...
    Prometheus timestamps are seconds-since-epoch. The CD hook writes ISO strings
...
def refresh_deploy_gauges() -> None:
    ...
    # the state may still be "running" until watch_dag records the terminal outcome).
